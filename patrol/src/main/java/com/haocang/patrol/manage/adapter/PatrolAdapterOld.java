...
public class PatrolAdapterOld extends BaseRecyclerAdapter<PatrolTaskListDTO> {
    private Context mContext;
    private Map<String, String> statusMap;

    private int changedItemPosition = -1;

    public PatrolAdapterOld(final Collection<PatrolTaskListDTO> collection,
                            final int layoutId) {
        super(collection, layoutId);
    }

    public PatrolAdapterOld(@LayoutRes final int layoutId,
                            final Context ctx) {
        super(layoutId);
        mContext = ctx;
