 ...
 * 修改时间：
 */
@Route(path = LibConfig.AROUTE_FAULT_SEVERITY)
public class PostFaultSeverityFragment extends Fragment implements View.OnClickListener, BaseAdapter.OnItemClickListener {
    /**
     * 列表适配器
     */
    ...
    public static final int FAULT_SEVERITY = 2003;//严重程度

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.from(getActivity()).inflate(R.layout.fragment_post_fault_severity, null);
        initView(view);
        return view;
    }
    ...
    private void initView(View view) {
        ...
        titleNameTv.setText(getString(R.string.fault_servity));
    ...
    private void renderList() {
        String[] nameArray = getActivity().getResources().getStringArray(R.array.fault_severitys);
        String[] idArray = getActivity().getResources().getStringArray(R.array.fault_severity_id);
    ...
    private void backCode(final LabelEntity entity) {
        Intent intent = new Intent();
        intent.putExtra("name", entity.getLabel());
        intent.putExtra("id", entity.getId() + "");
        getActivity().setResult(FAULT_SEVERITY, intent);
    }
